...
/// <summary>
/// Deterministic JSON Lines serializer for <see cref="DrawEvent"/>.
/// Hand-written to enforce:
///  - fixed key order
///  - invariant-culture number formatting
///  - "R" round-trip float format so two runs producing the same float produce the same text
/// </summary>
/// <remarks>
/// System.Text.Json would work but its default float formatting and property ordering have
/// moved between .NET versions. For a determinism-sensitive capture we serialize manually.
/// </remarks>
internal static class DrawEventWriter
{
    private static readonly CultureInfo Inv = CultureInfo.InvariantCulture;

    internal static void WriteHeader(TextWriter w, int ticks, int events, int dropped, string reason)
    {
        // Header line: "meta" namespaced so it's trivially filterable in the diff.
    ...
    internal static void WriteEvent(TextWriter w, in DrawEvent e)
    {
